...
RSpec.describe DirectoryGenerator do
  ...
  subject(:given_valid_yaml) { DirectoryGenerator::Runner.new(TEST_CASE_PATH) }
  ...
  context "when performing" do
    ...
    it "generates expected directories" do
      ...
      EXPECTED.each do |pair|
        path, content = pair.entries.first
        path_to_file = File.expand_path(File.join(example_target_root, path, content))
